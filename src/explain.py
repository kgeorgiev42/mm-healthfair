...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multimodal SHAP feature importance analysis.")
    parser.add_argument(
        "data_path",
        type=str,
    ...
    parser.add_argument(
        ...
        "-rd",
        type=str,
        help="Directory containing risk dictionary generated in evaluate.py.",
        default="../outputs/evaluation",
    )
    parser.add_argument(
        "--outcome",
    ...
    if len(glob.glob(os.path.join(args.model_dir, args.model_path, "*.pkl"))) == 0:
        print(f"No risk dictionary found at {args.model_path}. Exiting..")
        sys.exit()

    test_ids = pl.read_csv(os.path.join(args.ids_path, "testing_ids_" + args.outcome + ".csv")).select("subject_id").to_numpy().flatten()
    risk_dict = load_pickle(os.path.join(args.model_dir, args.model_path + ".pkl"))
    if args.group_by_attr:
        print("Reading attributes metadata for risk difference plotting...")
    ...
    if args.exp_mode == "global":
        ...
        for modality in modalities:
            if modality == "static":
                ...
                shap_scores, x_test = get_shap_values(model, batch, test_set, modality_type="tabular")
                get_shap_summary_plot(shap_scores, x_test, feature_names, 
                ...
                shap_dict.update({feature: shap_scores[:, idx] for idx, feature in enumerate(feature_names)})
                shap_ehr.update({feature: shap_scores[:, idx] for idx, feature in enumerate(feature_names)})
                ...
                shap_scores_vitals, x_test_vit = get_shap_values(model, batch, test_set, modality_type="ts-vitals")
                shap_scores_labs, x_test_lab = get_shap_values(model, batch, test_set, modality_type="ts-labs")
                ...
                get_shap_summary_plot(shap_scores, x_test, feature_names, 
                                      ...
                                      save_path=os.path.join(exp_path, f"shap_summary_{modality}.png"))
                shap_dict.update({feature: shap_scores[:, idx] for idx, feature in enumerate(feature_names)})
                shap_ts.update({feature: shap_scores[:, idx] for idx, feature in enumerate(feature_names)})
            elif modality == "notes":
                print("Processing explanations for notes modality...")
                feature_names = get_feature_names(test_set, modality_type="notes")
                shap_scores, x_test = get_shap_values(model, batch, test_set, modality_type="notes")
                get_shap_summary_plot(shap_scores, x_test, feature_names, 
                                      outcome=outcomes_disp[outcome_idx],
                                      fusion_type=fusion_method,
                                      modality=modality,
                                      ...
                                      save_path=os.path.join(exp_path, f"shap_summary_{modality}.png"))
                shap_dict.update({feature: shap_scores[:, idx] for idx, feature in enumerate(feature_names)})
                shap_notes.update({feature: shap_scores[:, idx] for idx, feature in enumerate(feature_names)})
        ### Plot PDP for a feature of interest
        if args.pdp_analysis:
            if args.pdp_feature not in shap_dict.keys():
                print(f"Feature {args.pdp_feature} not found in SHAP summary plot. Exiting...")
                sys.exit()
            print(f"Generating PDP for feature {args.pdp_feature}...")
            for attribute, attr_disp in zip(attributes, attr_disp):
                get_shap_partial_dependence(shap_dict, args.pdp_feature, 
                                            attribute, attr_disp,
                                            outcome=outcomes_disp[outcome_idx],
                                            fusion_type=fusion_method, 
        ...
        if args.global_group_by_risk:
            print(f"Generating risk difference plots (one vs rest) with target quantile {args.diff_risk_quantile}...")
            ...
            get_shap_group_difference(shap_diff, risk_dict, args.diff_risk_quantile, 
                                      feature_names=None, outcome=outcomes_disp[outcome_idx], fusion_type=None, 
                                    fusion_type=fusion_method, modalities=modalities,
                                    max_features=args.global_max_features,
                                    save_path=os.path.join(exp_path, f"shap_gdiff_{args.diff_modality}_rg_{args.diff_risk_quantile}_vs_rest.png"), figsize=(8, 5), verbose=True)
    
    if args.exp_mode == "local":
        print(f"Generating local-level SHAP explanations for {args.local_samples} subjects at risk quantile {args.local_risk_group}...")
        ...
        # Get feature names
        feature_names = get_feature_names(test_set, modality_type="tabular")
        # Plot SHAP summary plot for a single subject
        for i in range(args.local_samples):
            plt.figure(figsize=(12, 4))
            plt.title(
                f"Truth: {int(y_test[i])}, Predict: {int(y_hat[i])}, Prob: {round(prob[i], 2)}"
            )
            shap.bar_plot(
                shap_values[i][:, 1].values,
                feature_names=feature_names,
                max_display=20,
            )
            plt.show()